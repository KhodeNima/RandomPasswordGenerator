...
current_version = "Beta 3.0.0" 

color_blue = f"\33[34m"
color_red = f"\33[31m"
color_green = f"\33[32m"

value_type_error_message = "The argument type passed to the parameter ( {} ) Is not valid."
...
def reset_all_variables(_scope : object = OperationalVariables) -> None:
    """Give the value None to all the variables in the scope 

    Args:
        scope (object): Only accepts a class. ( Default set to OperationVariables )

    Raises:
        ValueError: If not the provided Scope is a class.

    ----- This Function is made for 1 case usage only , Do not use it oftenly.
    """

    if not inspect.isclass(_scope):
        raise ValueError(value_type_error_message ).format(f"{_scope=}".split('=')[0])
    
    _variables = [
        _variable for _variable in dir(_scope) 
        if
        not _variable.startswith('__') and
        not _variable.endswith('__') and 
        not _variable.__contains__('iterable') and
        not _variable.__contains__('length') and
        not _variable.startswith('input') and
        getattr(_scope , _variable) == True
    ]

    for _variable in _variables:
        setattr(_scope , _variable , False)